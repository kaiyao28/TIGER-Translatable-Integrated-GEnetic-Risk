...
prior <- 0.50              # prior in the target/test context
...
probabilities <- data.frame(
  ...
  BPC = bpc_probability(target_prs, K, prior, r2_liability),
  GenoPred = pain_probability(
    target_prs, reference_prs, r2_observed, K, Pt = prior,
    corrected = TRUE
  ),
  PAIR_summary = pair_probability_summary(
    target_prs, K, prior, r2_liability
  )
)

# Example damaging RV with OR 8 and protective RV with OR 0.4. P = 0.50 is the
# balanced-sample RV default; change it only for a justified application level.
p_damaging <- intrinsic_rv_probability(8, prevalence = prior)
p_protective <- intrinsic_rv_probability(0.4, prevalence = prior)
...
probabilities$BPC_plus_APOE <- apply_high_impact_probability(
  probabilities$BPC, apoe, apoe_reference
)
